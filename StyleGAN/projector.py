...
def project(
    ...
    *,
    ...
    for step in range(num_steps):
        ...
        optimizer.step()
        logprint(
            f"step {step+1:>4d}/{num_steps}: dist {dist:<4.2f} loss {float(loss):<5.2f} stdev: {improvement}"
        )
        ...
        # My auto early-stopping
        moving_avg.append(float(loss))
        if len(moving_avg) > 10:
            improvement = stdev(moving_avg)
