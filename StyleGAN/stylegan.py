...
# 5 Iterate through seeds to find minimal FID when projecting
# 6 Optimize projection speed when using pre-existing vectors
...
class Generation:
    def GUI(self):
        ...
        model = self.left_col.selectbox("Choose a model: ", model_list, 6)
        with open(f"models/{model}", "rb") as f:
            self.G = pickle.load(f)["G_ema"].cuda()  # torch.nn.Module
            f.close()

        self.vectorname = self.right_col.text_input(
            "Enter name for vector file", "my_vectors"
        )
        ...
        if self.method == "random":
            ...
            self.frequency = (
                self.left_col.slider("Choose a base frequency", 1, 100, 1, 1)
                / 100000
                * self.octave
            )
            self.modulation = (
                self.left_col.slider("Choose a modulation frequency", 1, 100, 1, 1)
                / 100000
                * self.octave
            )
            # self.amplitude = st.slider('Choose an amplitude', 0., 4., 0., .1)
            self.amplitude = 1
            self.cutoff = self.left_col.slider(
                "Choose a cutoff level", 1, self.G.z_dim, 1
            )
            self.cutoff_dir = self.left_col.checkbox("Invert cutoff")
            self.sinepolarity = self.left_col.checkbox("Sin/Cos")
            self.waveform = self.left_col.selectbox(
                "Choose a waveform", ["ramp", "sine"], 1
            )

    def _Sinewave(self, ramp):
        if self.sinepolarity:
            sine_array = np.sin(2 * np.pi * self.frequency * ramp)
        else:
            sine_array = np.cos(2 * np.pi * self.frequency * ramp)
        return sine_array
...
class Projection:
    def __init__(self):
        st.text(
            f"Projector extracts the features of the input image\n\
using a VGG16 model. It then generates a synthetic image,\n\
extracts its features, and compares the loss against the\n\
original. It does this for n-steps."
        )
    ...
    def Project(self):
        ...
        target = (
            torch.tensor(target_uint8.transpose([2, 0, 1]), device=device),
        )  # pylint: disable=not-callable

        self.vectors = (
            project(
                self.G, target[0], num_steps=self.num_steps, device=device, verbose=True
            )[-1]
            .unsqueeze(0)
            .cpu()
            .numpy()
        ...
        np.savez(f'out/{self.image_file.split(".")[0]}.npz', w=self.vectors)
...
class Synthesis:
    ...
    def GUI(self):
        ...
        self.vector_file = self.col.selectbox(
            "Choose a vector", vector_list, 0, key=self.idx
        )
...
def SaveVideo(G, vectors, length, name):
    ...
    for idx in range(len(vectors) - 1):
        transitions = np.concatenate(
            (
                transitions,
                np.linspace(vectors[idx][0], vectors[idx + 1][0], int(seconds * fps)),
            )
        )

    transitions = np.concatenate(
        (transitions, np.linspace(vectors[-1][0], vectors[0][0], int(seconds * fps)))
    )

    video_out = imageio.get_writer("out/{name}.mp4", mode="I", fps=60, codec="libx264")

    frame_idx = 0
    progress_bar = st.progress(0)
    for frame in transitions:
        progress_bar.progress(frame_idx/len(transitions))
